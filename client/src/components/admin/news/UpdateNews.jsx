...
const UpdateNews = () => {
    ...
    const getNews = async () => {
      try {
        ...
        setNewsData({
          ...
          date: formatDateInput(response.date),
        });
        setPreviewImage(
          `${uploadurl}/uploads/news/${response.img.replace(/\\/g, "/")}`
        );
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("task", `News ${newsData.name} is updated.`);
    ...
    const response = await updateRequest(`${baseurl}/news/${id}`, formData);
    if (response.error) {
